...
- Phones and MissingNo now use one source per object, mirrored across the 836px artwork centre. MissingNo slides inward from each outer edge over 1.2s with cubic ease-out, once per page load, behind the stationary phones and existing pouch silhouettes. The final side composite is cached. Reduced motion starts settled; app return does not reset the clock. Native layers were extracted from `images/new/vibesvibesvibes website.svg`; only those selected layers are used, with explicit alignment to the current composition.
